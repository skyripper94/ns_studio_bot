...
GRADIENT_INTENSITY_CURVE = 0.8   # плавность в верхней половине (больше = резче переход)
...
def openai_translate(text: str) -> str:
    ...
    try:
        ...
        system_prompt = (
            """Ты редактор коротких заголовков для обложек (СНГ).\n"
            "Задача: не дословный перевод, а сильная краткая формулировка, которая хорошо смотрится крупным КАПСОМ.\n"
            
            "Правила:\n"
            "1) Бренды оставляй на английском (SpaceX, Tesla, Apple)\n"
            "2) Адаптируй под естественный русский язык, не переводи дословно\n"
            "3) Избегай длинных слов (желательно не длиннее 12–13 букв). Заменяй на короткие синонимы.\n"
            "4) Сокращай валюту: billion→млрд., million→млн.\n"
            "5) Длина результата: 4–8 слов, максимум 42 символа (без учета пробелов — старайся короче"
            "6) Делай текст живым и понятным для СНГ\n"
            "7) Используй знаки препинания в русском языке, если это необходимо\n"
            "8) Дефис заменяй на тире, если это необходимо\n"
            "9) Если исходный текст уже на русском — НЕ "переводи", а сокращай и улучшай формулировку"
            "10) Верни ТОЛЬКО итоговую фразу, без кавычек, без пояснений, без точек в конце"
        ...
        resp = openai.ChatCompletion.create(
            ...
            messages=[
                ...
                {"role": "user", "content": f"Переведи и сделай короткий заголовок для обложки: {text}"},
            ],
            temperature=0.7,
            max_tokens=200,
        )
