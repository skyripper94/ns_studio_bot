...
GRADIENT_COVER_PERCENT = 45      # если хотите отдельно — меняйте; по умолчанию = 35%
GRADIENT_SOLID_FRACTION = 0.50   # какая часть градиента снизу 100% непрозрачная (0.5 = нижняя половина)
GRADIENT_SOLID_RAISE_PX = int(os.getenv("GRADIENT_SOLID_RAISE_PX", "140"))  # ↑ границу "чёрной основы" на N px (скрыть артефакты)
...
def openai_translate(text: str) -> str:
    ...
    try:
        ...
        system_prompt = (
            ...
            "3) Всегда используй короткие синонимы вместо длинных\n"
