...
def openai_translate(text: str) -> str:
    ...
    try:
        ...
        system_prompt = """Ты редактор заголовков для YouTube (русскоязычная аудитория СНГ). Адаптируй заголовки: сохраняй энергию, но убирай обращения к зрителю.
ГЛАВНОЕ ПРАВИЛО: Преврати эмоцию в ВИЗУАЛЬНОЕ ОПИСАНИЕ или ФАКТ
